...
class HookedKeyRenderer extends Visual {
	...
	override public function new(width1: Int, height1: Int, width2: Int, height2: Int, offsetX2: Int, offsetY2: Int) {
		...
		final fg = new RoundedRect(this.topColor, xInner, yInner, roundedCorner * scaleTo, width1 * scaleTo - this.offsetInner,
			height1 * scaleTo - this.offsetInner, this.borderInner, this.thicknessInner);
		fg.depth = 2;
		this.add(fg);
		// then draw the South portion of the keyshape
		final fg = new RoundedRect(this.topColor, xInner
			+ (offsetX2 / 2 * scaleTo), yInner
			+ offsetY2 / 2 * scaleTo, roundedCorner * scaleTo,
			width2 * scaleTo
			- this.offsetInner, height2 * scaleTo
		...
		bd.depth = 1;
		this.add(bd);
		final bd = new RoundedRect(this.bodyColor, offsetX2 / 2 * scaleTo, offsetY2 / 2 * scaleTo, roundedCorner * scaleTo, width2 * scaleTo,
			height2 * scaleTo, this.borderOuter, this.thicknessOuter);
		bd.depth = 1;
		this.add(bd);
