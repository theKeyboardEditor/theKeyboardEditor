...
class ColorBody extends Action {
	...
	override public function new(viewport: Viewport, colorees: Array<KeyRenderer>, color: ceramic.Color) {
		super();
	...
	override public function act(type: ActionType) {
		...
		trace ('colorees: ${this.colorees.length}');
		if (type != Redo) {
			trace('Initial action');
		...
		trace ('colored keys: ${this.coloredKeys.keys.length}');
		for (k in this.colorees) {
			// add to keyson:
			k.sourceKey.keysColor = '${0xff000000 + this.color}';
